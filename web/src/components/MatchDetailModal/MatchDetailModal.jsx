...
//
// 값은 [적중률%, 표본수, 같은 배당대의 나머지 경기 적중률%, z, 리그일치]. 한 칸당
// 표본 150건 미만이면 아예 넣지 않는다(못 믿을 값을 띄우지 않는다).
//   same    = 국·해가 같은 방향 (이름은 그중 국내 쪽 기준)
//   diff    = 방향이 갈렸을 때의 국내 쪽 / diffFor = 갈렸을 때의 해외 쪽
//   배당대  = 초기 줄은 초기배당 min(KW,KL), 배변 줄은 최종배당 min(EKW,EKL) 기준
//
...
const DIR_HIT = {
  init: {
    same: {
      정무: { '~1.5': [89, 3475, 89, -0.6, 0], '1.5~1.8': [80, 1635, 80, 0.0, 0], '1.8~2.2': [74, 649, 73, 0.5, 0] },
      정역: { '~1.5': [83, 1248, 82, 0.8, 0], '1.5~1.8': [74, 881, 74, 0.1, 0], '1.8~2.2': [70, 498, 71, -0.8, 0] },
      플핸무: { '1.5~1.8': [71, 516, 71, -0.0, 0], '1.8~2.2': [81, 2230, 78, 3.0, 1], '2.2+': [84, 1435, 84, 0.0, 0] },
      플핸승: { '~1.5': [79, 206, 75, 1.0, 1], '1.5~1.8': [76, 845, 75, 0.8, 0], '1.8~2.2': [79, 1876, 77, 2.0, 0], '2.2+': [78, 741, 79, -0.6, 0] },
      정: { '~1.5': [76, 2633, 69, 6.7, 1], '1.5~1.8': [56, 297, 54, 0.8, 0] },
      플: { '1.8~2.2': [58, 600, 56, 1.4, 0], '2.2+': [64, 531, 62, 1.0, 0] },
    },
    diff: {
      정무: { '~1.5': [85, 534, 89, -3.0, 1], '1.5~1.8': [79, 1033, 80, -0.2, 0], '1.8~2.2': [76, 1218, 73, 2.3, 1], '2.2+': [63, 321, 69, -2.2, 1] },
      정역: { '~1.5': [78, 230, 82, -1.5, 0], '1.5~1.8': [71, 593, 75, -1.8, 0], '1.8~2.2': [73, 1000, 71, 1.1, 1], '2.2+': [63, 305, 70, -2.7, 0] },
      플핸무: { '~1.5': [62, 154, 53, 2.2, 1], '1.5~1.8': [73, 624, 71, 1.0, 0], '1.8~2.2': [78, 1076, 78, -0.2, 0], '2.2+': [82, 310, 84, -1.0, 0] },
      플핸승: { '~1.5': [74, 915, 76, -0.9, 0], '1.5~1.8': [75, 1046, 75, -0.0, 0], '1.8~2.2': [78, 944, 77, 0.6, 0], '2.2+': [73, 202, 79, -1.9, 0] },
      정: { '~1.5': [70, 288, 71, -0.4, 0], '1.5~1.8': [59, 179, 54, 1.4, 0], '1.8~2.2': [42, 153, 44, -0.6, 0] },
      플: { '1.8~2.2': [59, 237, 56, 0.9, 0] },
    },
    diffFor: {
      정무: { '~1.5': [87, 655, 89, -1.9, 1], '1.5~1.8': [79, 1133, 80, -0.5, 0], '1.8~2.2': [76, 1310, 72, 2.7, 1], '2.2+': [69, 299, 68, 0.3, 0] },
      정역: { '~1.5': [83, 232, 82, 0.6, 0], '1.5~1.8': [73, 567, 74, -0.6, 0], '1.8~2.2': [73, 923, 71, 1.0, 0], '2.2+': [75, 335, 69, 2.4, 1] },
      플핸무: { '~1.5': [68, 176, 53, 3.9, 1], '1.5~1.8': [72, 715, 71, 0.5, 0], '1.8~2.2': [79, 1324, 78, 0.7, 0], '2.2+': [83, 424, 84, -0.4, 0] },
      플핸승: { '~1.5': [76, 859, 75, 0.5, 0], '1.5~1.8': [75, 1067, 75, 0.2, 0], '1.8~2.2': [77, 983, 77, -0.4, 0], '2.2+': [86, 203, 78, 2.5, 1] },
      정: { '~1.5': [75, 223, 71, 1.5, 0] },
    },
  },
  final: {
    same: {
      정무: { '~1.5': [86, 3329, 91, -6.9, 1], '1.5~1.8': [69, 1577, 82, -11.3, 1], '1.8~2.2': [59, 711, 74, -8.5, 1] },
      정역: { '~1.5': [77, 1256, 82, -4.8, 1], '1.5~1.8': [61, 872, 76, -9.5, 1], '1.8~2.2': [53, 570, 72, -9.9, 1] },
      플핸무: { '1.5~1.8': [56, 539, 72, -8.0, 1], '1.8~2.2': [71, 2092, 80, -9.3, 1], '2.2+': [78, 1278, 87, -7.4, 1] },
      플핸승: { '~1.5': [53, 295, 76, -9.2, 1], '1.5~1.8': [59, 865, 77, -11.4, 1], '1.8~2.2': [68, 1852, 79, -10.3, 1], '2.2+': [68, 733, 81, -8.0, 1] },
      정: { '~1.5': [71, 2641, 71, 0.4, 0], '1.5~1.8': [38, 344, 54, -6.1, 1] },
      플: { '1.8~2.2': [42, 582, 57, -6.9, 1], '2.2+': [50, 553, 64, -6.6, 1] },
    },
    diff: {
      정무: { '~1.5': [93, 647, 89, 3.0, 1], '1.5~1.8': [81, 1052, 79, 1.8, 1], '1.8~2.2': [71, 1111, 73, -1.3, 0], '2.2+': [60, 319, 69, -3.5, 1] },
      정역: { '~1.5': [81, 254, 82, -0.0, 0], '1.5~1.8': [72, 615, 75, -1.6, 1], '1.8~2.2': [68, 971, 72, -2.5, 1], '2.2+': [49, 295, 71, -7.6, 1] },
      플핸무: { '~1.5': [45, 162, 54, -2.2, 1], '1.5~1.8': [67, 671, 72, -2.7, 0], '1.8~2.2': [85, 1024, 78, 5.3, 1], '2.2+': [92, 407, 83, 4.8, 1] },
      플핸승: { '~1.5': [73, 891, 76, -1.7, 0], '1.5~1.8': [75, 973, 75, 0.2, 0], '1.8~2.2': [83, 914, 77, 4.1, 1], '2.2+': [82, 232, 78, 1.3, 0] },
      정: { '~1.5': [79, 335, 71, 3.2, 1], '1.5~1.8': [64, 176, 53, 2.7, 0], '1.8~2.2': [42, 164, 44, -0.5, 0] },
      플: { '1.8~2.2': [64, 255, 56, 2.5, 1] },
    },
    diffFor: {
      정무: { '~1.5': [91, 624, 89, 1.1, 0], '1.5~1.8': [67, 1046, 81, -10.0, 1], '1.8~2.2': [53, 1217, 75, -16.6, 1], '2.2+': [40, 374, 71, -12.4, 1] },
      정역: { '~1.5': [72, 213, 82, -3.5, 1], '1.5~1.8': [54, 607, 76, -11.9, 1], '1.8~2.2': [45, 941, 74, -19.0, 1], '2.2+': [40, 387, 72, -13.0, 1] },
      플핸무: { '~1.5': [30, 242, 54, -7.4, 1], '1.5~1.8': [49, 730, 73, -14.1, 1], '1.8~2.2': [70, 1227, 80, -7.5, 1], '2.2+': [83, 408, 84, -0.4, 0] },
      플핸승: { '~1.5': [57, 953, 77, -14.0, 1], '1.5~1.8': [60, 1074, 78, -12.4, 1], '1.8~2.2': [65, 914, 78, -9.1, 1], '2.2+': [74, 225, 79, -1.6, 1] },
      정: { '~1.5': [63, 259, 71, -2.8, 1] },
    },
  },
}
...
/** slot: 'same'(방향 일치) | 'diff'(갈렸을 때 국내 쪽) | 'diffFor'(갈렸을 때 해외 쪽) */
const DIR_RATE_WHERE = {
...
function DirRate({ phase, slot, name, band }) {
  const e = name && band ? DIR_HIT[phase]?.[slot]?.[name]?.[band] : null
  if (!e) {
    return (
      <span
  ...
  const tip = `${DIR_RATE_WHERE[slot](name)} '${name}', 정배배당 ${band}`
    ...
    <>
      ...
      {same && band !== false && (
        <>
          ...
          <DirRate phase={phase} slot="same" name={domName} band={band} />
        </>
      )}
    </>
  )
...
//   그래서 내 데이터에서는 숫자를 아예 안 띄운다(band=false) — 못 믿을 값을 띄우느니
...
function DirectionSummary({ row, scope }) {
  ...
  // 배당대는 그 줄이 실제로 쓴 배당으로 잡는다 — 초기 줄은 초기배당, 배변 줄은 최종배당.
