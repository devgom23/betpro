...
const COL_WIDTH = {
  ...
  // RT·적중·P — 예전 주석엔 "RtBadge.css/.cell-badge의 min-width로 어느 표에서든
  // 항상 같은 폭이 나와서 여기서 따로 안 잡는다"고 돼 있었는데, 그 min-width는
  // 뱃지가 실제로 그려질 때만 적용된다. 값이 없는 행(예정 경기라 RT 없음, 픽을
  // 아직 안 정해 P가 없음)은 뱃지 대신 빈 칸(mypick-blank, 폭 지정 없음)이 그려져
  // 그 컬럼만 아주 좁아진다 — 요일별 이번주 리스트처럼 표가 여러 개 나란히
  // 있으면(같은 컬럼인데) 값 있는 표와 없는 표의 폭이 서로 달라져 눈에 띈다.
  // 실측(빈 표를 만들어 콘텐츠만 재는 방식, 위 KW 폭 고정과 같은 방법):
  //   RT   값있음 61 / 빈값 17   → 63
  //   적중 값있음 61 / 빈값 29   → 63
  //   P    값있음 73~75 / 빈값 63 → 77
  // 내픽·의견·중요는 이미 빈값과 값 폭이 같아(버튼 자체 min-width가 충분해) 안 건드림.
  RT: 63, PICK_VERDICT: 63, MY_P: 77,
  // 벳(MY_BET)도 cell-badge/mypick-blank로 값 있음·없음을 가르는 같은 구조라
...
const PICK_VERDICT_MAP = {
  ...
  핸승: { hit: [1], insure: [] },
  핸무: { hit: [2], insure: [] },
  // 핸승핸무 — 핸승·핸무 둘 다 적중(보험 없음). 결과 칸으로는 '정'과 같다(2026-09-13 추가).
  핸승핸무: { hit: [1, 2], insure: [] },
  무: { hit: [3], insure: [] },
  역: { hit: [4], insure: [] },
  // ±2 핸디(2026-09-15 추가). RT 핸승(1) = 정배 2골차 이상 승이라 2플핸은 RT만으로 갈린다.
  '2플핸': { hit: [2, 3, 4], insure: [] },
  // 2플핸무 — RT 핸승 안에서 '정확히 2골차'(보험)와 '3골차 이상'(미적)을 스코어로 가른다.
  '2플핸무': { hit: [2, 3, 4], insure: [], insureIfMargin2: true },
}

// row: 스코어(HS/AS)가 필요한 픽(2플핸무)만 쓴다. 스코어가 없으면 판정을 비워 둔다.
export function computeAutoVerdict(pick, rt, row) {
  ...
  if (rule.insureIfMargin2 && code === 1) {
    const hs = toNum(row?.HS)
    const as = toNum(row?.AS)
    if (hs === null || as === null) return ''
    return Math.abs(hs - as) === 2 ? '보험' : '미적'
  }
  return '미적'
}
...
export function summarizeVerdicts(rows) {
  ...
  for (const row of rows) {
    const v = computeAutoVerdict(row.MY_PICK, row.RT, row)
...
const MY_PICK_DOG_GROUP = new Set(['플핸', '플핸무', '플핸승', '2플핸', '2플핸무', '무', '역', '무핸무'])
