// 이번주 TOP20 — 이번주 리스트 중 '당첨 확률'이 높은 경기 20개를 1위부터 세운다.
//
...
//   · 한 번 순위에 든 경기는 끝나도 남는다 — 명단(memberKeys, 서버 predlog.db에 저장)에
//     있는 끝난 경기는 후보로 계속 겨룬다. 다른 경기에 밀려 20위 밖으로 나가면 빠진다.
...
// rows: /api/week_list 행 전부 · memberKeys: 지금까지 순위에 든 경기 키(Set)
// 반환: { top: [{row, key, rank, played, score}], candidateCount }
export function rankTop20(rows, memberKeys) {
  const cands = []
