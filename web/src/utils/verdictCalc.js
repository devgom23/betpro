...
// '접전' 기준선 — 국내 정배배당(배변 EKW·EKL 중 낮은 쪽)이 이 값 이상이면 접전으로 본다.
// 2026-09-07 실측(6대리그 36,029경기): 정배배당이 커질수록 당첨률(플핸무)이 계단처럼
// 오른다 — 2.0~2.1 79.55% / 2.1~2.2 81.17% / 2.2~2.3 83.35% / 2.3~2.4 83.58%.
// 2.2·2.25·2.3 중 어디를 잘라도 통계적으로 구분되지 않아(z −1.1~−1.5), 경기 수를
// 가장 많이 남기면서 당첨률이 거의 안 떨어지는 2.25로 정했다(사용자 지정).
export const CLOSE_ODDS_CUT = 2.25

// 강추 2단계 — 뼈대(배변 판정 플핸무 + ★3)는 같고, 거기에 어느 길로 걸리느냐로 나뉜다.
//
//   초강추 = 뼈대 + 국≠해(oddsFavSplit)        n=842   적중 66.51% 당첨 85.39%
//   강추   = 뼈대 + 접전(국내 정배배당 2.25+)   n=1,383 적중 62.91% 당첨 84.16%
//   (뼈대만 = n=7,344, 적중 60.19% 당첨 81.67% — 두 길 모두 뼈대보다 확실히 위다)
//
// 2026-09-07 실측 근거:
//   · 국≠해 경기의 96.4%가 이미 접전 구간(정배배당 2.0~2.5)에 있다 — 두 길이 보는
//     것은 사실상 같은 현상('배당이 접전이라 정배가 애매하다')이라, 접전을 두 번째
//     길로 넣는 게 자연스럽다.
//   · 추가되는 쪽(강추)이 초강추보다 낮지만 z=−1.1 안팎으로 확정적 열세는 아니다.
//   · 리그 재현성 — 강추 4/6, 초강추 5/6(뼈대 평균 대비).
//   · 합치면 대상이 842건 → 2,200건 남짓(2.6배)으로 늘고 당첨률은 85.39% → 84.3%대.
//
// ★ 접전은 '배변(최신) 배당'으로 판정한다 — 초기 배당이 아니다(2026-09-07 실측).
//   경계를 넘나든 경기만 따로 재보니 방향이 뚜렷했다(뼈대+국=해 6,502건 기준):
//     초기 미달 → 배변 통과(2.21→2.26)  n=182  당첨 83.52%   ← 넣는 게 맞다
//     초기 통과 → 배변 미달(2.26→2.21)  n=110  당첨 76.36%   ← 빼는 게 맞다
//     (둘 다 통과 84.26% / 둘 다 미달 80.25%. 배변 미달로 빠진 쪽은 둘 다 미달보다도
//      낮다 — 둘 다 통과 대비 z=−2.14.) 배당이 접전에서 벗어나는 쪽으로 움직이는 것
//      자체가 나쁜 신호라는 뜻이다.
//   기준별 비교: 초기만 83.60%(n=1,311) / 배변 84.16%(n=1,383) / 둘 다 84.26%(n=1,201).
//   배변 기준이 경기도 더 많고 당첨률도 높아 그쪽으로 정했다. 강추 배지가 배변 판정에만
//   붙는다는 점, CLAUDE.md 4-1(배당이 갱신되면 그 배당으로 다시 판정한다)과도 맞는다.
//   배변 배당이 아직 없는 경기(16.1%)는 초기 배당으로 대신 본다.
//
// 화면 표시는 둘을 구분해서 보여주되(나중에 어느 길이 잘 맞았는지 따로 집계하려고),
// 리그표 '판정' 칸 이중밑줄과 배지 색(보라)은 둘 다 똑같이 쓴다(사용자 지정).
...
export function strongPickTier(row, verdict) {
  ...
  if (oddsFavSplit(row)) return '초강추'
  // 배변(EKW/EKL) 우선, 없으면 초기(KW/KL)로 대신한다.
  const kw = numOrNull(row.EKW) ?? numOrNull(row.KW)
  const kl = numOrNull(row.EKL) ?? numOrNull(row.KL)
  if (kw !== null && kl !== null && Math.min(kw, kl) >= CLOSE_ODDS_CUT) return '강추'
  return null
}

// 두 단계의 근거 문구 — 리그표 title과 상세보기 배지 tooltip이 같이 쓴다.
export const STRONG_TIER_TITLE = {
  초강추: '초강추 — 국내·해외 정배가 갈린 경기(국≠해)의 배변 플핸무 별3개.'
    + ' 6대리그 실측 당첨률 85.39%(적중 66.51%, n=842).',
  강추: `강추 — 접전(배변 국내 정배배당 ${CLOSE_ODDS_CUT} 이상)의 배변 플핸무 별3개.`
    + ' 6대리그 실측 당첨률 84.16%(적중 62.91%, n=1,383). 배변 배당이 없으면 초기 배당으로 봅니다.',
}
