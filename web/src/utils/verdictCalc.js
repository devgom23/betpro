...
// '접전' 기준선 — 국내 정배배당(KW·KL 중 낮은 쪽)이 이 값 이상이면 접전으로 본다.
...
//   강추   = 뼈대 + 접전(국내 정배배당 2.25+)   n=1,311 적중 62.47% 당첨 83.60%
...
//     길로 넣는 게 자연스럽다.
//   · 추가되는 쪽(강추)이 초강추보다 −1.79%p 낮지만 z=−1.12로 확정적 열세는 아니다.
//   · 리그 재현성 — 강추 4/6, 초강추 5/6(뼈대 평균 대비).
//   · 합치면 대상이 842건 → 2,153건(2.6배)으로 늘고 당첨률은 85.39% → 84.30%.
//
...
export const STRONG_TIER_TITLE = {
  ...
  강추: `강추 — 접전(국내 정배배당 ${CLOSE_ODDS_CUT} 이상)의 배변 플핸무 별3개.`
    + ' 6대리그 실측 당첨률 83.60%(적중 62.47%, n=1,311).',
}
